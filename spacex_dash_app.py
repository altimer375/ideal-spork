...
def get_pie_chart(entered_site,payload):
    ...
    if entered_site == 'ALL':
        ...
        fig1 = px.pie(filtered_df.groupby('class').size().reset_index(name='count'), values='count', names=['successful','failed'], 
        title="Launches per "+entered_site+ " site")
        fig2 = px.scatter(filtered_df[(filtered_df['Payload Mass (kg)'] > payload[0]) & (filtered_df['Payload Mass (kg)'] < payload[1])],
           x='Payload Mass (kg)',y='class',color="Booster Version Category",title='Corelation between Payload and success for' +entered_site+ " site with Payload selection")
        return fig1,fig2
